...
bool assembly_file(char* file, vector<int> correct){
    char* output = (char*)"a";
    char* input = file;
    Assembler* A = new Assembler(0, input, output);
    A->run();
...
void TestAssembler::test(){
    ...
    assert(assembly_file((char*)"test_files/LEX_ERR_99CHAR.s", correct));
    correct.clear();

    correct.push_back(LEX_ERR_INVALIDCHAR);
    assert(assembly_file((char*)"test_files/LEX_ERR_INVALIDCHAR.s", correct));
    correct.clear();
